...
export class Smk61DecisionTreeManager {
    ...
    goodsAssessmentHtml() {
        ...
        if (!rows.length) {
            return `<div class="opp-empty-note">Rakip başvurunun tam mal/hizmet kapsamı bulunamadı.</div>`;
        }

        return rows.map(row => `
            <div class="opp61-goods-card" data-opponent-class="${this.escape(row.opponentClassNo)}">
                <div class="opp61-goods-head">
                    <div>
                        <div class="opp61-class-chip">Rakip Sınıf ${this.escape(row.opponentClassNo)}</div>
                        <div class="opp61-goods-caption">İtiraz edilen kapsam</div>
                    </div>
                    ...
                    <label class="opp61-refusal-toggle">
                        ...
                        >
                        <span>Bu sınıf için ret talep et</span>
                    </label>
                </div>

                <details class="opp61-opponent-goods">
                    <summary>Rakip mal/hizmet metnini göster</summary>
                    <div>${this.escape(row.opponentText || 'Metin bulunamadı.')}</div>
                </details>

                <div class="opp61-field-grid mt-3">
                    <div>
                        <label class="form-label">Benzerlik Derecesi</label>
                        <select class="form-select opp61-similarity-select">
                            ${this.optionList(GOODS_SIMILARITY_OPTIONS, row.similarityLevel === 'not_assessed' ? '' : row.similarityLevel)}
                        </select>
                    </div>

                    <div>
                        <label class="form-label">Dayanılan Müstenit Sınıf(lar)</label>
                        <div class="opp61-prior-class-box">
                            ${this.priorClassOptionsHtml(row.matchedPriorClasses || [])}
                        </div>
                    </div>
                </div>

                <div class="mt-3">
                    <label class="form-label">Mal/Hizmet Benzerliği Kriterleri</label>
                    <div class="opp61-criteria-box">
                        ${this.criteriaHtml(row.criteria || [])}
                    </div>
                </div>

                <div class="mt-3">
                    <label class="form-label">Kısa Dosya Notu <span class="text-muted text-lowercase">(opsiyonel)</span></label>
                    <textarea
    ...
    signAssessmentHtml() {
        const sign = this.context?.formData?.signAssessment || {};

        return `
            <div class="opp61-field-grid">
                <div>
                    <label class="form-label">Ortak Unsur(lar)</label>
                    <input
                        type="text"
                        id="opp61CommonElements"
                        class="form-input"
                        value="${this.escape(sign.commonElements || '')}"
                        placeholder="Örn: Z / Z-ŞARJ / PREMIUM"
                    >
                </div>

                <div>
                    <label class="form-label">Farklı Unsur(lar)</label>
                    <input
                        type="text"
                        id="opp61Differences"
                        class="form-input"
                ...
                <div>
                    <label class="form-label">Ortak Unsurun Ayırt Ediciliği</label>
                    <select id="opp61Distinctiveness" class="form-select">
                        ${this.optionList(DISTINCTIVENESS_OPTIONS, sign.commonElementDistinctiveness || '')}
                    </select>
                </div>

                <div>
                    <label class="form-label">Ek Unsurların Rolü</label>
                    <select id="opp61AdditionalRole" class="form-select">
                        ${this.optionList(ADDITIONAL_ROLE_OPTIONS, sign.additionalElementsRole || '')}
                    </select>
                </div>

                <div>
                    <label class="form-label">Müstenit Markanın Baskın / Ayırt Edici Unsuru</label>
                    <input
                        type="text"
                ...
                <div>
                    <label class="form-label">Rakip Markanın Baskın / Ayırt Edici Unsuru</label>
                    <input
                        type="text"
                ...
                <div>
                    <label class="form-label">Ortak Unsurun Bağımsız Ayırt Edici Rolü</label>
                    <select id="opp61IndependentRole" class="form-select">
                        ${this.optionList(INDEPENDENT_ROLE_OPTIONS, sign.independentDistinctiveRole || '')}
                    </select>
                </div>
                ...
                <div>
                    <label class="form-label">Görsel Benzerlik</label>
                    <select id="opp61Visual" class="form-select">
                        ${this.optionList(SIGN_SIMILARITY_OPTIONS, sign.visualSimilarity || '')}
                    </select>
                </div>

                <div>
                    <label class="form-label">İşitsel Benzerlik</label>
                    <select id="opp61Aural" class="form-select">
                        ${this.optionList(SIGN_SIMILARITY_OPTIONS, sign.auralSimilarity || '')}
                    </select>
                </div>

                <div>
                    <label class="form-label">Kavramsal Benzerlik / Farklılık</label>
                    <select id="opp61Conceptual" class="form-select">
                        ${this.optionList(SIGN_SIMILARITY_OPTIONS, sign.conceptualSimilarity || '')}
                    </select>
                </div>

                <div>
                    <label class="form-label">Genel İzlenim Benzerliği</label>
                    <select id="opp61Overall" class="form-select">
                        ${this.optionList(SIGN_SIMILARITY_OPTIONS, sign.overallSimilarity || '')}
                    </select>
                </div>
            </div>

            <div class="mt-3">
                <label class="form-label">İşaret Analizi Notu <span class="text-muted text-lowercase">(opsiyonel)</span></label>
                <textarea id="opp61SignNote" class="form-control" rows="3">${this.escape(sign.note || '')}</textarea>
            </div>
        `;
    }
    ...
    bindEvents() {
        document.querySelectorAll('.opp61-refusal-check').forEach(input => {
            input.addEventListener('change', () => this.refreshLocalRefusalSummary());
        });

        document.getElementById('opp61SaveBtn')?.addEventListener('click', () => this.save());
    }

    refreshLocalRefusalSummary() {
        const summary = document.querySelector('.opp61-refusal-summary');
        if (!summary) return;

        const classes = [...document.querySelectorAll('.opp61-goods-card')]
            .filter(card => card.querySelector('.opp61-refusal-check')?.checked)
            .map(card => card.dataset.opponentClass)
            .filter(Boolean);

        summary.innerHTML = `
            <strong>Seçili ret kapsamı:</strong>
            ${classes.length
                ? classes.map(no => `<span>Sınıf ${this.escape(no)}</span>`).join('')
                : '<em>Henüz ret talep edilen sınıf seçilmedi.</em>'}
        `;
    }
    ...
    collectPayload() {
        ...
        const goodsAssessments = [...document.querySelectorAll('.opp61-goods-card')].map(card => ({
            opponentClassNo: Number(card.dataset.opponentClass),
            similarityLevel: card.querySelector('.opp61-similarity-select')?.value || 'not_assessed',
            matchedPriorClasses: [...card.querySelectorAll('.opp61-prior-class-check:checked')].map(el => el.value),
            criteria: [...card.querySelectorAll('.opp61-criteria-check:checked')].map(el => el.value),
            requestedRefusal: card.querySelector('.opp61-refusal-check')?.checked === true,
        ...
        return {
            ...
            signAssessment: {
                ...
                clientDominantElements: document.getElementById('opp61ClientDominant')?.value || '',
                opponentDominantElements: document.getElementById('opp61OpponentDominant')?.value || '',
                additionalElementsRole: document.getElementById('opp61AdditionalRole')?.value || '',
                independentDistinctiveRole: document.getElementById('opp61IndependentRole')?.value || '',
